...
public interface RestypeRepository {
	...
	final String R_RESTYPE = "SELECT restype_id, restype_name, restype_name_kh, restype_picture,"
			+ " date_added, date_modify, parentid_restypeid"
			+ " FROM  restypes ORDER BY date_added DESC OFFSET #{offset} limit #{limit}";
	@Select(R_RESTYPE)
	public ArrayList<Restypes> getAllRestype(Pagination paginatin);
	...
	String COUNT_RESTYPE = "SELECT COUNT(restype_id) FROM restypes";
